...
public sealed class ValidatorGenerator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor ZV0013 = new DiagnosticDescriptor(
        ...
        messageFormat: "Method '{0}' decorated with [CustomValidation] must not be generic, must have no parameters and must return IEnumerable<ValidationFailure>, ValidationFailure[] or ReadOnlySpan<ValidationFailure>",
        category: "ZeroAlloc.Validation",
        defaultSeverity: DiagnosticSeverity.Error,
        isEnabledByDefault: true);
    ...
    private static readonly DiagnosticDescriptor ZV0028 = new DiagnosticDescriptor(
        ...
        description:
            ...
            + "When, Unless and [SkipWhen] methods as instance.Method(). A static method, or one that "
            + "is private or protected, cannot be called that way, so the rule is skipped. Make the "
            + "method a public or internal instance method.");

    private static readonly DiagnosticDescriptor ZV0030 = new DiagnosticDescriptor(
        id: "ZV0030",
        title: "Validation method call that does not compile",
        messageFormat: "Method '{0}', used by {1}, cannot be called by the generated validator: {2}",
        category: "ZeroAlloc.Validation",
        defaultSeverity: DiagnosticSeverity.Error,
        isEnabledByDefault: true,
        description:
            "The generated validator calls a [Must] predicate as instance.Method(instance.Property), "
            + "and a When, Unless or [SkipWhen] condition as instance.Method(), used as a condition. "
            + "Each call is compiled in the generated file's context before it is emitted; one that "
...
#pragma warning restore EPS06
    ...
    private static void EmitFileHeader(
        ...
        Compilation compilation)
    {
        // Shared with MethodCallProbe, which compiles the validator's calls in this same context.
        GeneratedCalls.AppendPreamble(sb, namespaceName);
    ...
    /// <summary>
    /// ZV0017 for base members the generated validator cannot reach, and ZV0017, ZV0028 or ZV0030
    /// for the <c>When</c>, <c>Unless</c> and <c>[Must]</c> methods rules call. A static method is
    /// ZV0028 wherever it is declared, since widening it would not make it callable. An
    /// inaccessible instance method is ZV0028 on the model and ZV0017 on a base type, which may not
    /// be the author's to change. A call that does not compile otherwise is ZV0030. A usage
    ...
    private static void ReportInaccessibleBaseMemberDiagnostics(SourceProductionContext ctx, INamedTypeSymbol classSymbol, Compilation compilation)
    {
        ...
        foreach (var member in MemberWalker.GetMembersIncludingBase(classSymbol, compilation))
        {
            ...
            {
                // The base type's validator reports the call when it fails there too. A model that
                // declares its own member of that name can break a call that works from the base
    ...
    private static bool FailsFrom(Compilation compilation, INamedTypeSymbol reportingBase, IPropertySymbol prop, in ResolvedMethodCall call)
    {
        foreach (var fromBase in RuleEmitter.GetUnreachableMethodCalls(compilation, reportingBase, prop))
        {
            if (fromBase.Attribute == call.Attribute
                && string.Equals(fromBase.Usage, call.Usage, StringComparison.Ordinal))
                return true;
        }
        return false;
    }
    ...
    private static void ReportCustomValidationDiagnostics(SourceProductionContext ctx, INamedTypeSymbol classSymbol, Compilation compilation)
    {
        ...
        foreach (var member in candidates)
        {
            ...
            bool validSignature = !method.IsGenericMethod
    ...
    /// <summary>
    /// Reports a rule's call to a model method that the generated validator cannot make, which
    ...
    /// skip check is left out, so the model is always validated: an unusable skip condition fails
    /// the build rather than letting a model through unvalidated. A method inaccessible on a base
