...
**For library authors.** A `[Validate]` model's generated `{Model}Validator` is `public` only if the model itself is effectively public **and** every type its constructor takes is public too (see also [Inheritance — Accessibility](#inheritance--rules-declared-on-base-types)). The constructor takes each nested `[Validate]` model's validator — a scalar property or a collection element — as `ValidatorFor<TNested>`, so that nested model must be public; and it takes each `[ValidateWith]` validator by its own type, so that type must be public. Otherwise the validator is `internal`, even for an otherwise-public model. This is the unconditional default: a public model with a property whose type is an internal `[Validate]` model would otherwise generate a `public` validator whose constructor takes a less-accessible parameter — `CS0051` — so the generator makes the outer validator `internal` instead of emitting code that cannot compile. Only the direct dependencies count: `ValidatorFor<Customer>` is public whenever `Customer` is, even if `CustomerValidator` is itself internal because of an internal model further down. Before 2.0 the constructor took the nested validator's concrete type, so an internal model anywhere below made every validator above it internal too, and an internal `[ValidateWith]` validator on a public model failed with `CS0051`.
