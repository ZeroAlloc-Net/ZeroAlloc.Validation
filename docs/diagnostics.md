...
**Fix:** Change your code so the call no longer warns. For a nullability warning, let the method accept what it can receive, for example `IsKnownCode(string? code)`, or guard the rule with `When`, or use `[StopOnFirstFailure]` after `[NotNull]`. For an obsolete method, call its replacement. If the warning is expected, suppress ZV0032 where you can see it, with `#pragma warning disable ZV0032` around the attribute or `<NoWarn>$(NoWarn);ZV0032</NoWarn>` in the project. Under `TreatWarningsAsErrors`, ZV0032 is then the only thing left to deal with.
